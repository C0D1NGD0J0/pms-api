...
export class PaymentController {
  ...
  async listPayments(req: AppRequest, res: Response) {
    ...
    // Tenants can only see their own payments — resolve User._id to Profile._id in service
    if (currentuser?.client?.role === 'tenant') {
      filters.tenantUserId = currentuser.sub;
  ...
  async getPaymentStats(req: AppRequest, res: Response) {
    ...
    const result = await this.paymentService.getPaymentStats(cuid, tenantUserId);
