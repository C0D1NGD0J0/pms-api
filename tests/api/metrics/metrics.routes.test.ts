...
const mockMetricsController = {
  getDashboard: jest.fn((_req: Request, res: Response) => {
    res.status(httpStatusCodes.OK).json({
      ...
      data: {
        leases: {
          totalLeases: 5,
          leasesByStatus: { active: 4 },
          occupancyRate: 80,
          monthlyRentByCurrency: [{ currency: 'USD', total: 12000 }],
          expiringIn30Days: 1,
        },
        payments: {
          byCurrency: [
            { currency: 'USD', totalRevenue: 50000, monthRevenue: 5000, pendingAmount: 2000 },
          ],
          overdueCount: 1,
          totalCount: 20,
          onTimeRate: 90,
  ...
  getHistory: jest.fn((_req: Request, res: Response) => {
    res.status(httpStatusCodes.OK).json({
      ...
      data: [
        {
          metadata: { cuid: 'cuid123', metricType: 'payment' },
          measurements: { totalRevenue: 45000 },
